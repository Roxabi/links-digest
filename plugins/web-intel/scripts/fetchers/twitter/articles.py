...
_VIEWER_ENDPOINT = (
    "https://x.com/i/api/graphql/NimuplG1OB7Fd2btCLdBOw/Viewer?variables=%7B%7D&features=%7B%7D"
)
...
def validate_x_session(cookies_path: Optional[str] = None) -> tuple[bool, Optional[str]]:
    ...
    if missing:
        return False, (f"Missing required X cookies: {', '.join(missing)}\n{_COOKIE_REFRESH_GUIDE}")
    ...
    try:
        ...
        if resp.status_code == 403:
            ...
            return False, (f"X cookies are expired or invalid (HTTP 403).\n{_COOKIE_REFRESH_GUIDE}")
