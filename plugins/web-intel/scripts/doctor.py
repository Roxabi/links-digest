...
def check_playwright() -> tuple[str, str]:
    ...
    for browsers_path in candidates:
        if os.path.isdir(browsers_path):
            ...
            return "warn", (
                "playwright installed but chromium missing. Run: uv run playwright install chromium"
            )

    return "warn", (
        "playwright installed but chromium not detected. Run: uv run playwright install chromium"
    )
...
def check_youtube_transcript() -> tuple[str, str]:
    ...
    try:
        ...
        return "warn", ("youtube-transcript-api not installed. Run: uv sync --extra youtube")
...
def main() -> int:
    ...
    if opt_ok < opt_total:
        print()
        print("RESULT: OK — core ready. Install optional deps above to unlock all platforms.")
        print()
