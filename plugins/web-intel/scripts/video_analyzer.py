...
from gpu_detector import detect_gpu, detect_ollama, list_pulled_vision_models, select_best_model
...
def _extract_scene_frames(
    ...
    try:
        ...
        scene_cmd = [
            ...
            video_path,
            "-vf",
            f"select='gt(scene,{scene_threshold})',scale=-2:{scale_height}",
            "-vsync",
            "0",
            "-q:v",
            "3",
        ...
        if remaining_budget > 0 and duration and duration > 0:
            ...
            total_gap = sum(max(b - a - 3, 0) for a, b in zip(boundaries[:-1], boundaries[1:]))
        ...
        fps_cmd = [
            ...
            video_path,
            "-vf",
            f"fps={gap_fps},scale=-2:{scale_height}",
            "-q:v",
            "3",
            "-loglevel",
            "error",
...
def extract_frames(
    ...
    cmd = [
        ...
        video_path,
        "-vf",
        f"fps={fps},scale=-2:{scale_height}",
        "-q:v",
        "3",
        "-loglevel",
        "error",
...
def describe_frame_ollama(
    image_path: str, model: str, prompt: str = FRAME_PROMPT
) -> dict[str, Any]:
...
def analyze_video(
    ...
    print(
        f"  Extracted {extraction['frame_count']} frames ({extraction.get('mode')})",
        file=sys.stderr,
    )

    # --- Step 6: Batch describe frames ---
    print(
        f"Step 6/6: Describing {extraction['frame_count']} frames with {selected_model}...",
        file=sys.stderr,
    )
    ...
    avg_ms = (
        sum(d.get("inference_ms", 0) for d in successful) / len(successful) if successful else 0
    )
    result["stats"] = {
...
def main():
    ...
    parser.add_argument(
        ...
        type=float,
        default=1.0,
        help="Frames per second (when --no-scene-detection, default: 1)",
    )
    ...
    parser.add_argument(
        "--no-scene-detection",
        action="store_true",
        help="Disable scene detection, use uniform FPS instead",
    )
    parser.add_argument(
        "--scene-threshold",
        type=float,
