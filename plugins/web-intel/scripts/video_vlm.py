...
WEB_INTEL_SCRIPTS = Path(__file__).parent
...
def analyze(
    ...
    print(
        f"[3/4] VLM ({'two-pass' if two_pass else 'single-pass'}, deep={sorted(deep_kinds)})",
        file=sys.stderr,
    )
    ...
    for i, path in enumerate(frames):
        ...
        if two_pass:
            r1 = call_vlm(
                base_url, model, CLASSIFY_PROMPT, image_b64=img_b64, max_tokens=30, temperature=0.0
            )
