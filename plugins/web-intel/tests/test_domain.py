...
def test_fetch_result_frozen():
    ...
    r = FetchResult(
        success=True, content_type="twitter", url="https://x.com/test", data={"text": "hello"}
    )
...
def test_fetch_result_error():
    ...
    r = FetchResult(success=False, content_type="unknown", url="https://bad.com", error="Not found")
...
def test_fetch_result_defaults():
    ...
    r = FetchResult(
        success=True, content_type="webpage", url="https://example.com", data={"text": "content"}
    )
...
def test_web_intel_exceptions_hierarchy():
    from domain.exceptions import ContentParseError, FetchError, PluginError, WebIntelError
