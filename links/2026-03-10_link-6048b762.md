...
date: 2026-03-10
tags: ["scheduled tasks", "prompt automation", "task scheduling", "loop command", "interval polling"]
platform: web
author: "Anthropic"
summary: "Claude Code offers scheduled tasks via /loop, Desktop, and Cloud to run prompts automatically on intervals for polling and monitoring."
---
...
Use cloud tasks for work that should run reliably without your machine. Use Desktop tasks when you need access to local files and tools. Use /loop
for quick polling during a session.
Run a prompt repeatedly with /loop
The /loop
bundled skill is the quickest way to run a prompt on repeat while the session stays open. Both the interval and the prompt are optional, and what you provide determines how the loop behaves.
...
You can also pass another command as the prompt, for example /loop 20m /review-pr 1234
...
The interval can lead the prompt as a bare token like 30m
, or trail it as a clause like every 2 hours
. Supported units are s
for seconds, m
for minutes, h
for hours, and d
for days.
Seconds are rounded up to the nearest minute since cron has one-minute granularity. Intervals that don’t map to a clean cron step, such as 7m
or 90m
...
When you ask for a dynamic /loop
schedule, Claude may use the Monitor tool directly. Monitor runs a background script and streams each output line back, which avoids polling altogether and is often more token-efficient and responsive than re-running a prompt on an interval.
...
A bare /loop
runs this prompt at a dynamically chosen interval. Add an interval, for example /loop 15m
, to run it on a fixed schedule instead. To replace the built-in prompt with your own default, see Customize the default prompt with loop.md.
On Bedrock, Vertex AI, and Microsoft Foundry, /loop
...
A loop.md
file replaces the built-in maintenance prompt with your own instructions. It defines a single default prompt for bare /loop
, not a list of separate scheduled tasks, and is ignored whenever you supply a prompt on the command line. To schedule additional prompts alongside it, use /loop 
...
The file is plain Markdown with no required structure. Write it as if you were typing the /loop
...
Edits to loop.md
take effect on the next iteration, so you can refine the instructions while a loop is running. When no loop.md
...
For one-shot reminders, describe what you want in natural language instead of using /loop
...
Each scheduled task has an 8-character ID you can pass to CronDelete
...
All times are interpreted in your local timezone. A cron expression like 0 9 * * *
...
The offset is derived from the task ID, so the same task always gets the same offset. If exact timing matters, pick a minute that is not :00
or :30
, for example 3 9 * * *
instead of 0 9 * * *
...
CronCreate
accepts standard 5-field cron expressions: minute hour day-of-month month day-of-week
...
for Saturday. Extended syntax like L
, W
, ?
, and name aliases such as MON
or JAN
is not supported.
...
in your environment to disable the scheduler entirely. The cron tools and /loop
become unavailable, and any already-scheduled tasks stop firing. See Environment variables for the full list of disable flags.
Limitations
